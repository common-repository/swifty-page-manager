...
if( ! class_exists( 'Swifty_TGM_Plugin_Activation' ) ) {
    ...
    class Swifty_TGM_Plugin_Activation
    {
         ...
         * @since 1.0.0
         *
         * @var Swifty_TGM_Plugin_Activation
         */
         ...
         * does the stgmpa_init action hook, and hooks in the interactions to init.
         *
         * @since 1.0.0
         *
        ...
        public function __construct()
        {
            ...
            $this->strings = array(
                'page_title' => nlf__( 'Install Required Plugins', 'swifty-page-manager' ),
                'menu_title' => nlf__( 'Install Plugins', 'swifty-page-manager' ),
                'installing' => nlf__( 'Installing Plugin: %s', 'swifty-page-manager' ),
                'oops' => nlf__( 'Something went wrong.', 'swifty-page-manager' ),
                'notice_can_install_required' => _n_noop( 'This theme requires the following plugin: %1$s.', 'This theme requires the following plugins: %1$s.', 'swifty-page-manager' ),
                'notice_can_install_recommended' => _n_noop( 'This theme recommends the following plugin: %1$s.', 'This theme recommends the following plugins: %1$s.', 'swifty-page-manager' ),
                'notice_cannot_install' => _n_noop( 'Sorry, but you do not have the correct permissions to install the %s plugin. Contact the administrator of this site for help on getting the plugin installed.', 'Sorry, but you do not have the correct permissions to install the %s plugins. Contact the administrator of this site for help on getting the plugins installed.', 'swifty-page-manager' ),
                'notice_can_activate_required' => _n_noop( 'The following required plugin is currently inactive: %1$s.', 'The following required plugins are currently inactive: %1$s.', 'swifty-page-manager' ),
                'notice_can_activate_recommended' => _n_noop( 'The following recommended plugin is currently inactive: %1$s.', 'The following recommended plugins are currently inactive: %1$s.', 'swifty-page-manager' ),
                'notice_cannot_activate' => _n_noop( 'Sorry, but you do not have the correct permissions to activate the %s plugin. Contact the administrator of this site for help on getting the plugin activated.', 'Sorry, but you do not have the correct permissions to activate the %s plugins. Contact the administrator of this site for help on getting the plugins activated.', 'swifty-page-manager' ),
                'notice_ask_to_update' => _n_noop( 'The following plugin needs to be updated to its latest version to ensure maximum compatibility with this theme: %1$s.', 'The following plugins need to be updated to their latest version to ensure maximum compatibility with this theme: %1$s.', 'swifty-page-manager' ),
                'notice_cannot_update' => _n_noop( 'Sorry, but you do not have the correct permissions to update the %s plugin. Contact the administrator of this site for help on getting the plugin updated.', 'Sorry, but you do not have the correct permissions to update the %s plugins. Contact the administrator of this site for help on getting the plugins updated.', 'swifty-page-manager' ),
                'install_link' => _n_noop( 'Begin installing plugin', 'Begin installing plugins', 'swifty-page-manager' ),
                'activate_link' => _n_noop( 'Begin activating plugin', 'Begin activating plugins', 'swifty-page-manager' ),
                'return' => nlf__( 'Return to Required Plugins Installer', 'swifty-page-manager' ),
                'dashboard' => nlf__( 'Return to the dashboard', 'swifty-page-manager' ),
                'plugin_activated' => __( 'Plugin activated successfully.', 'swifty-page-manager' ),
                'activated_successfully' => __( 'The following plugin was activated successfully:', 'swifty-page-manager' ),
                'complete' => nlf__( 'All plugins installed and activated successfully. %1$s', 'swifty-page-manager' ),
                'dismiss' => __( 'Dismiss this notice', 'swifty-page-manager' ),
            );
            ...
            do_action_ref_array( 'stgmpa_init', array( $this ) );
         ...
         * @since 2.0.0
         *
         * @see Swifty_TGM_Plugin_Activation::admin_menu()
        ...
        public function install_plugins_page()
        {
            ...
            $plugin_table->menu_url = $this->menu_url;
...
//                } ?>
        ...
        protected function do_plugin_install()
        {
            ...
            if( isset( $_GET[ 'plugin' ] ) && ( isset( $_GET[ 'stgmpa-install' ] ) && 'install-plugin' == $_GET[ 'stgmpa-install' ] ) ) {
                ...
                if( empty( $complete ) ) {
                    echo '<p>' . sprintf( $this->strings[ 'complete' ], '<a href="' . network_admin_url() . '" title="' . nlf__( 'Return to the Dashboard', 'swifty-page-manager' ) . '">' . nlf__( 'Return to the Dashboard', 'swifty-page-manager' ) . '</a>' ) . '</p>';
        ...
        public function notices()
        {
            ...
            if( ! empty( $message ) ) {
                ...
                foreach( $message as $type => $plugin_groups ) {
                    ...
                    $rendered .= '<p>' . sprintf( translate_nooped_plural( $this->strings[ $type ], $count, 'swifty-page-manager' ), $imploded, $count ) . '</p>';
                }

                // Setup variables to determine if action links are needed.
                $show_install_link = $install_link ? '<a href="' . add_query_arg( 'page', $this->menu, $this->menu_url ) . '">' . translate_nooped_plural( $this->strings[ 'install_link' ], $install_link_count, 'swifty-page-manager' ) . '</a>' : '';
                $show_activate_link = $activate_link ? '<a href="' . add_query_arg( 'page', $this->menu, $this->menu_url ) . '">' . translate_nooped_plural( $this->strings[ 'activate_link' ], $activate_link_count, 'swifty-page-manager' ) . '</a>' : '';
        ...
        /**
         * Determine if we're on the STGMPA Install page.
         *
         * @since 2.1.0
         *
    ...
    $stgmpa = Swifty_TGM_Plugin_Activation::get_instance();
...
if( ! class_exists( 'STGMPA_List_Table' ) ) {
    ...
    class STGMPA_List_Table extends WP_List_Table
    {
         ...
         *
         * The constructor also grabs a copy of $instance from the STGMPA class
         * and stores it in the global object Swifty_TGM_Plugin_Activation::$instance.
         *
         * @since 2.2.0
         *
            ...
            foreach( Swifty_TGM_Plugin_Activation::$instance->plugins as $plugin ) {
                ...
                if( ! empty( $plugin[ 'source' ] ) ) {
                    ...
                    if( preg_match( '|^http(s)?://|', $plugin[ 'source' ] ) || ( strpos( $plugin[ 'source' ], 'swiftyget:' ) === 0 ) ) {
                        $table_data[ $i ][ 'source' ] = nlf__( 'Private Repository', 'swifty-page-manager' );
                        // The plugin is pre-packaged with the theme.
                    } else {
                        $table_data[ $i ][ 'source' ] = nlf__( 'Pre-Packaged', 'swifty-page-manager' );
                    }
                } // The plugin is from the WordPress repository.
                else {
                    $table_data[ $i ][ 'source' ] = nlf__( 'WordPress Repository', 'swifty-page-manager' );
                }

                $table_data[ $i ][ 'type' ] = isset( $plugin[ 'required' ] ) && $plugin[ 'required' ] ? nlf__( 'Required', 'swifty-page-manager' ) : nlf__( 'Recommended', 'swifty-page-manager' );

                if( ! isset( $installed_plugins[ $plugin[ 'file_path' ] ] ) ) {
                    $table_data[ $i ][ 'status' ] = sprintf( '%1$s', nlf__( 'Not Installed', 'swifty-page-manager' ) );
                } elseif( is_plugin_inactive( $plugin[ 'file_path' ] ) ) {
                    $table_data[ $i ][ 'status' ] = sprintf( '%1$s', nlf__( 'Installed But Not Activated', 'swifty-page-manager' ) );
                }
         ...
         * Swifty_TGM_Plugin_Activation class.
         *
         * Loops through the registered plugins looking for $name. If it finds it,
         * it returns the $data from that plugin. Otherwise, returns false.
         *
        ...
        public function column_plugin( $item )
        {
            ...
            if( ! isset( $installed_plugins[ $item[ 'file_path' ] ] ) ) {
                $actions = array(
                    'install' => sprintf(
                        '<a href="%1$s" title="' . nlf__( 'Install', 'swifty-page-manager' ) . ' %2$s">' . nlf__( 'Install', 'swifty-page-manager' ) . '</a>',
                        wp_nonce_url(
                            add_query_arg(
                                array(
            ...
            elseif( is_plugin_inactive( $item[ 'file_path' ] ) ) {
                $actions = array(
                    'activate' => sprintf(
                        '<a href="%1$s" title="' . nlf__( 'Activate', 'swifty-page-manager' ) . ' %2$s">' . nlf__( 'Activate', 'swifty-page-manager' ) . '</a>',
                        add_query_arg(
                            array(
        ...
        public function no_items()
        {
            ...
            printf( nlf__( 'No plugins to install or activate. <a href="%1$s" title="Return to the Dashboard">Return to the Dashboard</a>', 'swifty-page-manager' ), network_admin_url() );
            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
        ...
        public function get_columns()
        {
            ...
            $columns = array(
                ...
                'plugin' => nlf__( 'Plugin', 'swifty-page-manager' ),
                'source' => nlf__( 'Source', 'swifty-page-manager' ),
                'type' => nlf__( 'Type', 'swifty-page-manager' ),
                'status' => nlf__( 'Status', 'swifty-page-manager' )
            );
        ...
        public function get_bulk_actions()
        {
            ...
            $actions = array(
                'stgmpa-bulk-install' => nlf__( 'Install', 'swifty-page-manager' ),
                'stgmpa-bulk-activate' => nlf__( 'Activate', 'swifty-page-manager' ),
            );
...
//                }
            ...
            if( 'stgmpa-bulk-activate' === $this->current_action() ) {
                ...
                if( is_wp_error( $activate ) ) {
                    ...
                    printf( '<div id="message" class="updated"><p>%1$s %2$s</p></div>', _n( 'The following plugin was activated successfully:', 'The following plugins were activated successfully:', $count, 'swifty-page-manager' ), $imploded );
                }
        ...
        if( isset( $_GET[ 'page' ] ) && Swifty_TGM_Plugin_Activation::$instance->menu === $_GET[ 'page' ] ) {
            ...
            if( ! class_exists( 'TGM_Bulk_Installer' ) ) {
                ...
                class TGM_Bulk_Installer extends WP_Upgrader
                {
                     ...
                     * set to true for the STGMPA class.
                     *
                     * @since 2.2.0
                     *
                    ...
                    public function install_strings()
                    {
                        ...
                        $this->strings[ 'no_package' ] = nlf__( 'Install package not available.', 'swifty-page-manager' );
                        $this->strings[ 'downloading_package' ] = nlf__( 'Downloading install package from <span class="code">%s</span>&#8230;', 'swifty-page-manager' );
                        $this->strings[ 'unpack_package' ] = nlf__( 'Unpacking the package&#8230;', 'swifty-page-manager' );
                        $this->strings[ 'installing_package' ] = nlf__( 'Installing the plugin&#8230;', 'swifty-page-manager' );
                        $this->strings[ 'process_failed' ] = nlf__( 'Plugin install failed.', 'swifty-page-manager' );
                        $this->strings[ 'process_success' ] = nlf__( 'Plugin installed successfully.', 'swifty-page-manager' );
                    ...
                    public function activate_strings()
                    {
                        ...
                        $this->strings[ 'activation_failed' ] = nlf__( 'Plugin activation failed.', 'swifty-page-manager' );
                        $this->strings[ 'activation_success' ] = nlf__( 'Plugin activated successfully.', 'swifty-page-manager' );
            ...
            if( ! class_exists( 'TGM_Bulk_Installer_Skin' ) ) {
                ...
                class TGM_Bulk_Installer_Skin extends Bulk_Upgrader_Skin
                {
                    ...
                    public function add_strings()
                    {
                        ...
                        if( Swifty_TGM_Plugin_Activation::$instance->is_automatic ) {
                            $this->upgrader->strings[ 'skin_upgrade_start' ] = nlf__( 'The installation and activation process is starting. This process may take a while on some hosts, so please be patient.', 'swifty-page-manager' );
                            $this->upgrader->strings[ 'skin_update_successful' ] = nlf__( '%1$s installed and activated successfully.', 'swifty-page-manager' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . nlf__( 'Show Details', 'swifty-page-manager' ) . '</span><span class="hidden">' . nlf__( 'Hide Details', 'swifty-page-manager' ) . '</span>.</a>';
                            $this->upgrader->strings[ 'skin_upgrade_end' ] = nlf__( 'All installations and activations have been completed.', 'swifty-page-manager' );
                            $this->upgrader->strings[ 'skin_before_update_header' ] = nlf__( 'Installing and Activating Plugin %1$s (%2$d/%3$d)', 'swifty-page-manager' );
                        } // Default installation strings.
                        else {
                            $this->upgrader->strings[ 'skin_upgrade_start' ] = nlf__( 'The installation process is starting. This process may take a while on some hosts, so please be patient.', 'swifty-page-manager' );
                            $this->upgrader->strings[ 'skin_update_failed_error' ] = nlf__( 'An error occurred while installing %1$s: <strong>%2$s</strong>.', 'swifty-page-manager' );
                            $this->upgrader->strings[ 'skin_update_failed' ] = nlf__( 'The installation of %1$s failed.', 'swifty-page-manager' );
                            $this->upgrader->strings[ 'skin_update_successful' ] = nlf__( '%1$s installed successfully.', 'swifty-page-manager' ) . ' <a onclick="%2$s" href="#" class="hide-if-no-js"><span>' . nlf__( 'Show Details', 'swifty-page-manager' ) . '</span><span class="hidden">' . nlf__( 'Hide Details', 'swifty-page-manager' ) . '</span>.</a>';
                            $this->upgrader->strings[ 'skin_upgrade_end' ] = nlf__( 'All installations have been completed.', 'swifty-page-manager' );
                            $this->upgrader->strings[ 'skin_before_update_header' ] = nlf__( 'Installing Plugin %1$s (%2$d/%3$d)', 'swifty-page-manager' );
                        }
                    ...
                    public function bulk_footer()
                    {
                        ...
                        if( empty( $complete ) ) {
                            echo '<p>' . sprintf( Swifty_TGM_Plugin_Activation::$instance->strings[ 'complete' ], '<a href="' . network_admin_url() . '" title="' . nlf__( 'Return to the Dashboard', 'swifty-page-manager' ) . '">' . nlf__( 'Return to the Dashboard', 'swifty-page-manager' ) . '</a>' ) . '</p>';
                            echo '<style type="text/css">#adminmenu .wp-submenu li.current { display: none !important; }</style>';
                        }
